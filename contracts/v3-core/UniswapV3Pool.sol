...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    // 需要跟踪现在的价格和对应的 tick, 需要频繁地读写，放在一个struct
    struct Slot0 {
        // the current price
        ...
        returns (Position.Info storage position, int256 amount0, int256 amount1)
    {
        ...
        if (params.liquidityDelta != 0) {
            // 高于现价的价格区间，该价格区间的下界 tick 高于现价 tick
            if (_slot0.tick < params.tickLower) {
                ...
                // 低于现价的价格区间，该价格区间的上界 tick 低于现价 tick
    ...
    //  Uniswap V2 的池子给予 LP-token 作为提供流动性的交换，被称作 铸造(mint)
    //  V3 没有这种行为，但是仍然保留了同样的名字
    //  parms：token所有者的地址 tick上界和下界的 希望提供流动性的数量
    ...
    {
        require(amount > 0);
        // 合约更新 ticks 和 positions 的mapping，返回需要用户质押代币的数量
        ...
        // 从用户储获取相应Token
        // 预期调用者为合约地址，因为普通用户地址无法实现 callback 函数。
        // 使用 callback 函数看起来很不用户友好，但是这能够让合约计算 token 的数量——这非常关键，因为无法信任用户
    ...
    struct SwapState {
        ...
        int256 amountSpecifiedRemaining; // 有正负值
    ...
    /// @inheritdoc IUniswapV3PoolActions
    // params: token接收者 交换的方向true表示用token0交换token1 特定金额 价格sqrt
    function swap(
        address recipient,
        bool zeroForOne,
        int256 amountSpecified,
        uint160 sqrtPriceLimitX96, // 滑点保护
        bytes calldata data
        ...
        // 检查sqrtPriceLimitX96，确认限制金额在LP的价格区间里并且合理，
        // zeroForOne为true limit价格在[最小P，现价]之间
        require(
            zeroForOne
        ...
        // 根据tick来进行while遍历，tickSpacing设定tick之间距离，距离越大，越节省gas
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            // 这个函数里考虑跨tick的情况，计算这一步交易的费用
            (state.sqrtPriceX96, step.amountIn, step.amountOut, step.feeAmount) = SwapMath.computeSwapStep(
                state.sqrtPriceX96,
                // 对sqrtPriceNextX96进行滑点检查
            ...
            // 计算当前价格区间能够提供的流动性的数量，以及交易达到的目标价格
            if (exactInput) {
                // step.amountIn 是这个价格区间可以从用户手中买走的token数量
        ...
        // 当我们需要在跨过 tick 时更新流动性。全局的更新是在循环之后
